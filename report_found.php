...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($description) || empty($category_id) || empty($location_id) || empty($expiration_date)) {
        ...
        $stmt = $db->prepare("INSERT INTO Item (Item_ID, Creator_ID, Category_ID, Location_ID, Description, Priority, Status, Item_Type, Reported_Date, Expiration_Date) VALUES (?, ?, ?, ?, ?, ?, 'reported', 'found', ?, ?)");
        ...
        if ($stmt->execute([$item_id, $_SESSION['user_id'], $category_id, $location_id, $description, $priority, $reported_date, $expiration_date])) {
            ...
            $success = 'Found item reported successfully!' . ($bounty_amount > 0 ? ' A bounty of ' . number_format($bounty_amount) . ' TK has been offered for this item.' : '');
        } else {
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="location_id" class="form-label">Location: item was founded... *</label>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <small class="form-text text-muted">After this date, the item may be donated if not claimed.</small>
                                </div>
                            </div>

                            <div class="mb-3">
                                <label for="item_image" class="form-label">Item Photo (Recommended)</label>
                                <input type="file" class="form-control" id="item_image" name="item_image"
                                    accept="image/*">
                                <small class="form-text text-muted">Upload a photo to help the owner identify their
                                    item.</small>
                            </div>
